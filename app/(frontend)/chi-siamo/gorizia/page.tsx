...
import { vipGorizia, otherRolesGorizia, documentsGorizia, direzioneProvincialeGorizia, assembleaProvincialeGorizia } from "@/data/gorizia";
import { sanityFetch } from "@/sanity/lib/fetch";
import { SanityDocument } from "@sanity/client";
import { postsByFederationQuery } from "@/sanity/lib/queries";

export default async function GoriziaPage() {
  const posts = await sanityFetch<SanityDocument[]>({ query: postsByFederationQuery, params: { federation: "Gorizia" } });
  return (
    <>
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="space-y-16">
              <div className="bg-white rounded-lg shadow-md p-8 border border-gray-300">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {direzioneProvincialeGorizia.map((member) => (
                    <div key={member.name} className="bg-gray-100 p-4 rounded-lg text-center flex items-center justify-center space-x-2">
                      <User className="h-5 w-5 text-orange-500" />
                      <p className="text-base font-medium text-gray-800">{member.name}</p>
                    </div>
              ...
              <div className="bg-white rounded-lg shadow-md p-8 border border-gray-300">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {assembleaProvincialeGorizia.map((member) => (
                    <div key={member.name} className="bg-gray-100 p-4 rounded-lg text-center flex items-center justify-center space-x-2">
                      <User className="h-5 w-5 text-orange-500" />
                      <p className="text-base font-medium text-gray-800">{member.name}</p>
                    </div>
